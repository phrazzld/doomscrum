...
- [ ] From the UI (not hand-edited TOML), a user selects a local repo path,
...
- [ ] Per-repo state dirs: renders/events/dispatches for repo A never bleed
      into repo B; spend tracked per repo.

## Children
1. Repo picker + sync UX (covers the demo-day flow: select olympus, sync,
   feed populates).
2. Foreign-repo dispatch verification: worktree, branch, push, PR remote
