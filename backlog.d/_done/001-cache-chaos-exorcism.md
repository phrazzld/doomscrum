...
## User
Operators reviewing agent-delivered web app changes in the local DoomScrum feed,
...
Renders are generated state under `.doomscrum/renders/<spec-sha>/`, and the
...
- `README.md` documents the provenance contract:
  `.doomscrum/renders/<spec-sha>/<render-id>.json` records provider, model, spec
  sha256, storyboard hash, latency, and job id, while deleting `.doomscrum/`
...
- A generation test proves that a second successful generation for the same PRD
  ...
  readable from `.doomscrum/renders/<spec-sha>/` for audit.
